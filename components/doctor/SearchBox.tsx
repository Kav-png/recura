...
export function SearchBox({ patients }: { patients: SearchPatient[] }) {
  ...
  return (
    <div ref={containerRef} className="relative hidden md:block w-[220px] lg:w-[280px]">
      <div className="flex items-center gap-2.5 surface-sm rounded-xl px-4 py-2.5">
      ...
      {open && query.trim() && (
        <div className="absolute top-full mt-2 left-0 right-0 surface rounded-xl overflow-hidden z-50">
          {results.length === 0 ? (
